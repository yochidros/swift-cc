...
assert 4 'main() {1+3;}'
assert 1 'main() {2-1;}'
assert 0 'main() {1+2+3-6;}'
assert 1 'main() {1 + 10 - 10;}'
assert 100 'main() {255 + 100 - 100 - 155;}'
assert 1 'main() {1;}'
assert 10 'main() {- -10;}'
assert 10 'main() {- - +10;}'
assert 15 'main() {1+2+3*4;}'
assert 1 'main() {1+2/3*4;}'
assert 2 'main() {+1-2++3;}'
assert 1 'main() {if(0) return 2; return 1;}'
assert 2 'main() {if(1) return 2; return 1;}'
assert 1 'main() {if(0) return 2; else return 1;}'
assert 2 'main() {if(1) return 2; else return 1;}'
...
assert 6 'main() {i=0; j=0; for (i=0; i <= 3; i =i+1)  {j=j+i;} return j;}'
assert 0 'main() {i=0; j=0; for (i=0; i <= 3; i =i+1)  {j=j+i; j=0;} return j;}'
assert 3 'main() {i=0; j=0; for (i=0; i <= 3; i =i+1)  {j=j+i; j=0; j=i;} return j;}'

assert 3 'main() { return ret3(); }'
assert 8 'main() { return ret8(); }'

assert 1 'main() { return retArg1(1); }'
assert 3 'main() { return retArg2(2, 1); }'
assert 6 'main() { return retArg3(1, 2, 3); }'
assert 10 'main() { return retArg4(1, 2, 3, 4); }'
assert 15 'main() { return retArg5(1, 2, 3, 4, 5); }'
assert 21 'main() { return retArg6(1, 2, 3, 4, 5, 6); }'

assert 32 'main() { return ret32(); } ret32() { return 32; }'

assert 7 'main() { return add2(3, 4); } add2(x, y) { return x+y; }'
assert 1 'main() { return sub2(3, 2); } sub2(x, y) { return x-y; }'
